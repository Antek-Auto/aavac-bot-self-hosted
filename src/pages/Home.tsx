...
const Home = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      const {
        data
      } = await supabase.rpc("get_demo_display_settings");
        ...
        <div className="relative z-10 flex flex-col items-center gap-10 w-full max-w-lg text-center">
          <div className="space-y-4 animate-fade-in">
            ...
            <h1 className="text-4xl md:text-5xl font-bold tracking-tight">
              ...
              <span className="text-foreground">AAVAC Bot
            </span>{" "}
          ...
          <div className="grid gap-4 w-full">
            {settings?.enable_chat !== false && <div className="glass rounded-xl p-4 text-left">
                <h3 className="font-semibold text-foreground mb-2">💬 Chat Mode</h3>
                <p className="text-sm text-muted-foreground">
                  Type your questions and get instant AI responses
                </p>
              </div>}
            {settings?.enable_voice !== false && <div className="glass rounded-xl p-4 text-left">
